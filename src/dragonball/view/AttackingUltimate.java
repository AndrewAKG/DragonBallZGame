...
//		S.setSize(200, 70);
	...
	public void updateU(PlayableFighter x){
		S.removeAllItems();
		for (int i = 0; i < x.getUltimateAttacks().size(); i++) {
			if(S.getItemCount()<x.getUltimateAttacks().size()){
			S.addItem(x.getUltimateAttacks().get(i).getName());
			}
			else{
				break ;
			}
